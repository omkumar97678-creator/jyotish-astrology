...
export default function CompatibilityToday({ selected }) {
  ...
  const comp = compSource[s.en] || compSource.Aries || {};
    ...
    >
      <div className="mb-4">
        ...
        <p className="text-xs" style={{ color: 'var(--col-moonstone-dim)' }}>
          Best and challenging signs for {s.en} today
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 p-3 rounded-lg" style={{ background: 'rgba(42, 171, 168, 0.08)', border: '1px solid rgba(42, 171, 168, 0.25)' }}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            {bestList.map((item) => (
              ...
              >
                <span className="zodiac-sign-symbol unselected" style={{ fontSize: '13px' }}>{item.sym}</span>
                <span>{item.name}</span>
              </span>
            ))}
          </div>
        ...
        {/* Challenging Row */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 p-3 rounded-lg" style={{ background: 'rgba(245, 158, 11, 0.06)', border: '1px solid rgba(245, 158, 11, 0.25)' }}>
